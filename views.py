...
def manage(request):
    ...
    if request.method == "POST":
        ...
        news=[]
        dic = {-1:'审批未通过',0:'分配',1:'院级审核',2:'校级审批',3:'负责人审阅',4:'test'}
...
def myApplication(request):
    if request.method == "POST":
        if request.POST.get("type") == "init":
            dic = {'-1':'审批未通过','0': '待分配', '1': '学院审核中', '2': '学校审批中', '3': '负责人审核中', '4': '申请完成'}
            user = get_user(request)
        ...
        if request.POST.get("type") =="draft":
            ...
            if fieldname == 1: #学习类
                ...
                arole = 3
                signrole =4
            elif fieldname == 2: #运动类
                crole = 5
                arole = 6
                signrole =7
            elif fieldname == 3:  #场馆类
                crole = 8
                arole = 9
                signrole = 10
            ...
            cnames = models.MyUser.objects.filter(role_id=crole)
            # administration.countersign1 = models.MyUser.objects.get(username=countersign1)
            administration.call = 1
            # print(cnames)
            temp = ''
            for cname in cnames:
                temp = temp + cname.username + ','
                message1 = models.message(username=cname, applinum=contract, missionnum=1)
                message1.save()
            administration.countersign1 = temp

            # # mail(administration.countersign1.email,'尊敬的用户您好！ 您需要会签一份合同，请及时登录查看！')

            #审批分配
            anames = models.MyUser.objects.filter(role_id=arole)
            administration.aall = 1
            # print(anames)
            temp2 = ''
            for aname in anames:
                temp2 = temp2 + aname.username + ','
                message1 = models.message(username=aname, applinum=contract, missionnum=3)
                message1.save()

            administration.approval1 = temp2


            # #   mail(administration.approval1.email, '尊敬的用户您好！ 您需要审核一份合同，请及时登录查看！')

            snames = models.MyUser.objects.filter(role_id=signrole)
            temp2 = ''
            for sname in snames:
                temp2 = temp2 + sname.username + ','
                message1 = models.message(username=sname, applinum=contract, missionnum=3)
                message1.save()

            administration.sign = temp2
...
def myManageContract(request):
    if request.method == "POST":
        ...
        if type == "init":
            ...
            # print(user)
            results = []
            temps = models.Administration.objects.all()
            # print(temps)
            for temp in temps:
                name1 = temp.countersign1
                namelists = name1.split(',')
                for namelist in namelists:
                    if user.username == namelist:
                        results.append(temp)
            for temp in temps:
                name2 = temp.approval1
                namelists2 = name2.split(',')
                for namelist2 in namelists2:
                    if user.username == namelist2:
                        results.append(temp)
            for temp in temps:
                name3 = temp.sign
                namelists3 = name3.split(',')
                for namelist3 in namelists3:
                    if user.username == namelist3:
                        results.append(temp)

            contracts = []
            dic = {'-1':'审批未通过','0': '待分配', '1': '院级审核中', '2': '校级审批中', '3': '负责人审核中', '4': '申请完成'}
            for result_ in results:
                ...
                if contract['stateNum'] =='-1':
                    ...
                    name1 = result_.countersign1
                    namelists = name1.split(',')
                    for namelist in namelists:
                        if user.username == namelist:
                            contract_1 = contract.copy()
                            contract_1['myMissionNum'] = '1'
                            contract_1['myMission'] = '院级审核'
        ...
        if type =="countersign":
            ...
            name1 = administration.countersign1
            namelists = name1.split(',')
            for namelist in namelists:
                if user.username == namelist:
        ...
        if type =="approval":
            ...
            name2 = administration.approval1
            namelists = name2.split(',')
            for namelist in namelists:
                if user.username == namelist:
...
def setContract(request):
    if request.method == "POST":
        if (request.POST.get("type") == "init"):
            ...
            firstresult =models.MyUser.objects.filter(Q(role__firstfun=True) & Q(role__gymfun=True))
            secondresult=models.MyUser.objects.filter(Q(role__secondfun=True) & Q(role__gymfun=True))
            thirdresult=models.MyUser.objects.filter(Q(role__thirdfun=True) & Q(role__gymfun=True))
            users = []
    ...
    if request.method == "POST":
        ...
        if type == "init":
            ...
            for actname in actnames:
                ...
                results2 = models.Enrollment.objects.filter(activityid=actname.activityid)
                ...
                for result2 in results2:
                    enrollment = {
                        'stateNum': result2.is_paid,
                    }
                    enrollments.append(enrollment)

            return JsonResponse({'activities': activities, 'enrollments': enrollments})
        if type == "pay":
            user_model = models.MyUser.objects.get(username=get_user(request).username)
            select_payment = models.Enrollment.objects.get(username=user_model,
                                                           activityid=request.POST.get("activityid"))
            select_payment.is_paid = True
            select_payment.save()
            msg = 'success'
            return JsonResponse({'msg': msg})
        if type == "add":
